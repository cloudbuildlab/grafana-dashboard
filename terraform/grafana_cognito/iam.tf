...
data "aws_iam_policy_document" "ecs_execution_ssm_cognito" {
  statement {
    effect    = "Allow"
    actions   = ["ssm:GetParameter", "ssm:GetParameters"]
...
resource "aws_iam_role_policy" "ecs_execution_ssm_cognito" {
  name   = "${var.environment}-${local.app_name}-ssm-cognito"
  role   = aws_iam_role.ecs_execution.id
  policy = data.aws_iam_policy_document.ecs_execution_ssm_cognito.json
}
...
# Minimal EFS permissions: mount and write as task user (472). No ClientRootAccess (container runs non-root).
