...
variable "private_subnet_ids" {
  description = "List of private subnet IDs for the EC2 instance (need NAT or VPC endpoints for Docker/SSM)."
  type        = list(string)
}
...
variable "instance_type" {
  description = "EC2 instance type for Grafana (e.g. t3.small)."
  type        = string
  default     = "t3.small"
}

variable "grafana_image" {
  description = "Container image for Grafana. Pin to a specific tag for reproducibility and security (e.g. grafana/grafana-oss:12.1.1)."
  type        = string
  default     = "grafana/grafana-oss:12.1.1"
}
...
variable "grafana_admin_password" {
  description = "Optional: Grafana admin password. Stored in SSM Parameter Store (SecureString) and injected at runtime. Leave empty for default admin/admin."
  type        = string
  default     = ""
  sensitive   = true
}
