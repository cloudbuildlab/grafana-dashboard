...
resource "aws_iam_role" "cognito_pre_token" {
  name = "${var.environment}-${local.app_name}-cognito-pre-token"
...
resource "aws_lambda_function" "cognito_pre_token" {
  function_name    = "${var.environment}-${local.app_name}-cognito-pre-token"
  ...
  tags = merge(var.tags, { Name = "${var.environment}-${local.app_name}-cognito-pre-token" })
}
